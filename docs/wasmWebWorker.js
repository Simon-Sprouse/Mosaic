...
// Compute base path of the worker script
const basePath = self.location.origin + self.location.pathname.replace(/\/[^/]*$/, '');

// Import the compiled glue code
importScripts(`${basePath}/wasm/image_module.js`);

// Instantiate the module
Module({
  locateFile: (path) => {
    if (path.endsWith('.wasm')) {
      return `${basePath}/wasm/image_module.wasm`;
    }
    return path;
  }
}).then((instance) => {
